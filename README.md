...
```bash
python test.py --checkpoint runs/uvit3/BUSI/seed2981/best.pth --batch-size 8 --workers 4
```
...
```bash
python test.py --checkpoint runs/uvit3/BUSI/seed2981/best.pth \
  --image 'data/BUSI/images/benign (16).png' --output output/example_mask.png
```

Replace the image path with any readable image. The output is a binary PNG (0 or 255). A checkpoint is generated by training and is not bundled with this source release.
